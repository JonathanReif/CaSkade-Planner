...
"""
This test checks if the planner can create a plan for drilling a hole into a raw cylinder and before transporting to drilling module with MPS500. 
This ontology file contains three capabilities (supply, drilling and transport) of the supply module, drilling module and conveyor module from MPS500 that is supposed to be executed in three happenings.
"""
class TestSupplyDrilling:
	...
	def test_create_and_solve(self):
		ontology_files = [ 
			...
			os.path.join(os.getcwd(), "capabilities", "manufacturing", "mps500", "tests", "required-capabilities", "Supply_Drilling.ttl"), 
		...
		try:
			...
			max_happenings = 3
			planner: CaskadePlanner = CaskadePlanner("http://www.hsu-hh.de/aut/ontologies/lab/MPS500/required/SupplyToDrilling#SupplyToDrilling") 
			...
			for property in expected_plan.plan_steps[2].capability_appearances[0].outputs:
				if property_drilling_depth == property.property.iri:
					assert property.value == 8, "Drilling depth after drilling should be 8"
					property_drilling_depth_output = True
				elif property_drilling_diameter == property.property.iri:
					assert property.value == 10, "Drilling diameter after drilling should be 10"
...
if __name__ == '__main__':
	TestSupplyDrilling().test_create_and_solve()