...
"""
This test checks if the planner can create a plan for assembling a product with MPS500. 
This ontology file contains one capability (assembly) of the assembly module from MPS500 that is supposed to be executed in one happening.
"""
class TestAssembly:
	...
	def test_create_and_solve(self):
		ontology_files = [ 
			...
			os.path.join(os.getcwd(), "capabilities", "manufacturing", "mps500", "tests", "required-capabilities", "AssemblyOnly.ttl"), 
		...
		try:
			...
			max_happenings = 1
			planner: CaskadePlanner = CaskadePlanner("http://www.hsu-hh.de/aut/ontologies/lab/MPS500/required/Assembly#AssemblyOnly") 
			...
			assert expected_plan.plan_steps[0].capability_appearances[0].capability_iri == "http://www.hsu-hh.de/aut/ontologies/lab/MPS500/AssembleCylinder#AssembleCylinder", "First capability should be AssembleCylinder"
...
if __name__ == '__main__':
	TestAssembly().test_create_and_solve()